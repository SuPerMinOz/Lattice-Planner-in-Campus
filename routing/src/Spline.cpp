...
Spline1D::Spline1D(VEC_1D x_, VEC_1D y_) : x(x_), y(y_), a(y_)
{
    ...
    {
        d.emplace_back((c[i + 1] - c[i]) / std::max(3.0 * h[i], 1e-6));  // nan
        b.emplace_back((a[i + 1] - a[i]) / std::max(h[i], 1e-6) - h[i] * (c[i + 1] + 2.0 * c[i]) / 3.0);  // nan
    }
};
...
Eigen::VectorXd Spline1D::calc_B()
{
    ...
    {
        B(i + 1) = 3.0 * (a[i + 2] - a[i + 1]) / std::max(h[i + 1], 1e-6) - 3.0 * (a[i + 1] - a[i]) / std::max(h[i], 1e-6);  // nan
    }
    return B;
};
